...
type TextAnnotations = Awaited<
  ReturnType<typeof client.textDetection>
>[number]["textAnnotations"];
...
function getDescriptionsByY(
  array: NonNullable<TextAnnotations>
...
export async function detectText(filePath: string) {
  try {
    ...
    return await getTotal(joinedValues);
  } catch (error) {
...
export async function getTotal(parsedReceipt: Record<string, string> | object) {
  try {
